...
public class PagerSlidingTabStrip extends HorizontalScrollView {
    ...
    private int lastPaddingRight = 0;
    ...
    public PagerSlidingTabStrip(Context context, AttributeSet attrs, int defStyle) {
        ...
        lastPaddingRight = (int) TypedValue.applyDimension(TypedValue.COMPLEX_UNIT_DIP, lastPaddingRight, dm);
        ...
        lastPaddingRight =
                a.getDimensionPixelSize(R.styleable.PagerSlidingTabStrip_pstsLastPaddingRight, lastPaddingRight);
        indicatorPadding =
                a.getDimensionPixelSize(R.styleable.PagerSlidingTabStrip_pstsIndicatorPadding, indicatorPadding);
        textAllCaps = a.getBoolean(R.styleable.PagerSlidingTabStrip_pstsTextAllCaps, textAllCaps);
        inCenter = a.getBoolean(R.styleable.PagerSlidingTabStrip_pstsSelectOnCenter, inCenter);
        mFadeEnabled = a.getBoolean(R.styleable.PagerSlidingTabStrip_pstsmFadeEnabled, mFadeEnabled);
        ...
        smoothScrollWhenClickTab =
                a.getBoolean(R.styleable.PagerSlidingTabStrip_pstsSmoothScrollWhenClickTab, smoothScrollWhenClickTab);
        a.recycle();

        tabsContainer = new LinearLayout(context);
        tabsContainer.setOrientation(LinearLayout.HORIZONTAL);
        tabsContainer.setLayoutParams(new LayoutParams(LayoutParams.MATCH_PARENT, LayoutParams.MATCH_PARENT));
        tabsContainer.setPadding(0, 0, lastPaddingRight, 0);
    ...
    public void setSelectOnCenter(boolean inCenter) {
    ...
    private void scrollToChild(int position, int offset) {
        ...
        try {
            ...
            if (position == 0) { //第一个特殊处理
                smoothScrollTo(0, 0);
            }

            // 左侧
            if (tabsContainer.getChildAt(position).getLeft() < getScrollX()) { // 左侧的被卡住
                newScrollX = tabsContainer.getChildAt(position).getLeft();
            }

            // 右侧
            if (getScrollX() + this.getMeasuredWidth() < tabsContainer.getChildAt(position).getLeft() + tabsContainer
                    .getChildAt(position).getMeasuredWidth() + lastPaddingRight) { // 右侧的被卡住
                newScrollX = tabsContainer.getChildAt(position).getLeft() + tabsContainer
                        .getChildAt(position).getMeasuredWidth() - this.getMeasuredWidth() + lastPaddingRight;
            }
        ...
        //        }
        //                LogUtils.error("view", "getLeft--->" + tabsContainer.getChildAt(position).getLeft());
        //                LogUtils.error("view", "scroll--->" + getScrollX());
        //
    ...
    protected void onDraw(Canvas canvas) {
        ...
        if (currentPosition >= tabsContainer.getChildCount()) {
            currentPosition = tabsContainer.getChildCount() - 1;
        }
    ...
    private class PageListener implements OnPageChangeListener {
        ...
        public void onPageSelected(int position) {
            if (oldPosition >= tabsContainer.getChildCount()) {
                oldPosition = tabsContainer.getChildCount() - 1;
            }
    ...
    public void setIndicatorPadding(int indicatorPadding) {
        this.indicatorPadding = (int) TypedValue.applyDimension(
                TypedValue.COMPLEX_UNIT_DIP, indicatorPadding, getResources().getDisplayMetrics());
        invalidate();
    }
